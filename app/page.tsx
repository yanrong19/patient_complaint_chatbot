...
export default function Home() {
  ...
  const sessionComplaintIdRef = useRef<string | null>(null);
  ...
  const sendMessage = useCallback(
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            if (event.type === "agent_start") {
              ...
              // Capture complaint card (only on first filing)
              if (event.complaint) {
    ...
    [isStreaming, messages, addWorkbenchStep, updateWorkbenchStep, enqueueTTS]
  );
  ...
  return (
    <div ref={containerRef} className="flex h-screen bg-slate-950 text-white overflow-hidden select-none">
      {/* Left: Dialog Panel */}
